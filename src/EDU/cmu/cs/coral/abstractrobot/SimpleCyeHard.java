...
import CMVision.JCMVision;
import EDU.cmu.cs.coral.cye.JCyeComm;
import EDU.cmu.cs.coral.cye.JCyeSrv;
import EDU.gatech.cc.is.abstractrobot.HardObject;
...
public class SimpleCyeHard extends Simple implements SimpleCye, HardObject {
    ...
    private final int XWIDTH = 80;
    private final double FOV = Math.PI * 80.0 / 180;
    private final int YWIDTH = 60;
    private final double PIXEL_METER_CONVERSION = 0.01; // start with one pixel is one centimeter - highly unlikely and might need lookup
    private final int MAX_BALL_PIXEL = 70;
    private final double[] DistanceLookUp = {10.275, 3.163, 3.163, 3.163, 2.4518, 2.147, 1.9438, 1.7406, 1.639, 1.5374,
            ...
            0.2166, 0.2166};
    private final double[][] PixelToDistance = new double[160][120];
    /**
    ...
    private final long last_VisualObjectst = 0;
     ...
     * @throws Exception If unable to configure the hardware.
     */
    ...
    public void takeStep() {
        ...
        if (keep_running) {
            ...
            double x = Srv.GetLastX() * CYEUNITSTOMETERS; // conversion from 1/10 of foot to meters
            double y = Srv.GetLastY() * CYEUNITSTOMETERS;
...
//				proportion = 0.0;
			...
			*/
                    ...
                    double new_x = delta_displacement.x;
                    double new_y = delta_displacement.y;
                    new_Position = new Vec2(position.x + new_x, position.y + new_y);
                    velocity = desired_speed * base_speed;
