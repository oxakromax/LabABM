...
public class JohnCyeSim extends SimpleCyeSim implements SimulatedObject,
	...
	*/
    ...
    public SimulatedObject[] getLandmarks() {
        if (landmarks == null) {
            ...
            for (i = 0; i < landmarkRectangle.length; i++) {
                ...
                newx = landmarks[i].getPosition().x - landmarks[i].getRadius();
                newy = landmarks[i].getPosition().y + landmarks[i].getRadius();
    ...
    public double[] getMovementDistParams() {
        ...
        params[1] = Math.sqrt(Math.pow(.02, 2.0) * (mvstep.r)); //move mag std dev
