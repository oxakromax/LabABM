...
public final class Heap extends AbstractCollection implements PriorityQueue {
    ...
    private location getLocation(Object ob) {
    ...
    private location getLocation(Object ob, int hint) {
        if (EntryMap != null) {
    ...
    public Iterator iterator() {
        return new Iterator() {
            ...
            final int changeCount = Heap.this.changeCount;
    ...
    private boolean alteredKey(location l) {
        changeCount++;
        if (l.loc == 0) {
            smallDownHeap(l);
        } else {
    ...
    private void upHeap(location hole) {
        if (elementCount > 1) {
...
*/
    ...
    private void largeDownHeap(location hole) {
        if (elementCount <= 1)
            return;
    ...
    private void smallDownHeap(location hole) {
        if (elementCount <= 1)
            return;
