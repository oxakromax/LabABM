...
public class NewtonCalibrate {
    ...
    public static void main(String[] args) {
        try {
            ...
            int num = newt.getNumVis(Newton.CHANNEL_A);
            int[] rows = new int[num];
            int[] areas = new int[num];
            newt.getY(Newton.CHANNEL_A, rows);
            newt.getArea(Newton.CHANNEL_A, areas);
            for (int j = 0; j < num; j++) {
            ...
            for (double i = 0; i < 3.2; i += .2) {
                ...
                for (int k = 0; k < 20; k++) newt.read_frame();
                num = newt.getNumVis(Newton.CHANNEL_A);
                rows = new int[num];
                areas = new int[num];
                newt.getY(Newton.CHANNEL_A, rows);
                newt.getArea(Newton.CHANNEL_A, areas);
            ...
            for (double i = -1.2; i < 1.0; i += .1) {
                ...
                areas = new int[num];
                newt.getX(Newton.CHANNEL_A, cols);
                newt.getArea(Newton.CHANNEL_A, areas);
                robot.get_rc();
                for (int j = 0; j < num; j++) {
